...
class NuminaMath(object):
    ...
    def __init__(self, model: str, base_url: str, api_key: str = None, stream: bool = True, **kwargs):

        self.client = OpenaiAPI(
            model=model,
            base_url=base_url,
            api_key=api_key,
            **kwargs
    ...
    def process_all(self, data_list, max_workers=8):
        ...
        def call_api_parallel(sample_d: dict):
            ...
            try:
                resp = self.client.predict(prompt, stream=self.stream)
            except Exception as e:
                print(f'Got error when calling API with data id: {sample_d["id"]}, error message: {e}')
                return None
            ...
            sample_d['model'] = self.client.model
